...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for(n<-ns\\"a"; hr<-n\"@href"; h<-hr.head) yield createURL(Some(u), h.toString()) // TO BE IMPLEMENTED
    ...
    // 9 points.
    // TO BE IMPLEMENTED
    for { g <- getURLContent(u); f <- MonadOps.asFuture(getLinks(g))} yield f
  }
